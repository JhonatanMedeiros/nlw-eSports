...
export const CreateAdModal = ({ games, onCloseDialog }: CreateAdModalProps) => {
	...
	const onSubmit: SubmitHandler<Ad> = async (formData) => {
		...
		try {
			await api.post(`/gamdes/${data.game}/ad`, data);
			notify('Anúcio criado com sucesso', 'success');
			onCloseDialog();
		} catch (error) {
			notify('Ocorreu um erro ao criar o anúncio', 'error');
		}
	};
